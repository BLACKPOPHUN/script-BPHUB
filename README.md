# All Script
*** Key ***

* 7952

**** :) ****
# โซเชียล
*** Tiktok ***

* @_jwdes

*** YouTube ***

* @jwdes

*** Thank you ***

สคริป BP (สร้างโดย BlackpopHub)
```batch
-- สร้างโดยคนไทย
-- คัดลอกไปวางใช้ได้เลย

loadstring(game:HttpGet(("https://raw.githubusercontent.com/BLACKPOPHUN/script-BPHUB/refs/heads/main/Script/BPHUB"), true))()

-- ข้อควรระวัง
-- รีตัวเมนูอาจหาย
```
99 day
```batch
loadstring(game:HttpGet("https://raw.githubusercontent.com/BLACKPOPHUN/script-BPHUB/refs/heads/main/Script/FoxnameHub-99Day"))()
```
99day Thai เมนูภาษาไทย
```batch
-- ไม่ต้องแปลภาษานะครับ คัดลอกเลยย

    loadstring(game:HttpGet("https://raw.githubusercontent.com/BLACKPOPHUN/script-BPHUB/refs/heads/main/Script/TH-99Day"))()

-- ใช้อย่างระมัดระวัง!!
```
99คืนออโต้หาเพชร
```batch
loadstring(game:HttpGet(("https://raw.githubusercontent.com/BLACKPOPHUN/script-BPHUB/refs/heads/main/Script/Auto-farm-Gem-99Day"), true))()
```
Fisch ตกปลา
```batch
loadstring(game:HttpGet(("https://raw.githubusercontent.com/BLACKPOPHUN/script-BPHUB/refs/heads/main/Script/Auto-Farm-Fishing"), true))()
```
# Script All
*** Script Roblox ***

* ซัพพอร์ต
* Delta
* Xeno
* และอื่นๆที่ยังไม่ได้ทดสอบ

*** BlackpopHub ***
# link
* กันก็อป script

--> [here](https://wearedevs.net/obfuscator)
* ลิ้งทรี Tree

--> [here](https://linktr.ee/_jwdes)

![image](https://raw.githubusercontent.com/TPR002/BPHub/refs/heads/main/%E0%B8%AD%E0%B8%B8%E0%B8%9B%E0%B8%81%E0%B8%A3%E0%B8%93%E0%B9%8C/%E0%B8%A3%E0%B8%B9%E0%B8%9B%E0%B9%82%E0%B8%8A%E0%B8%A7%E0%B9%8C.png)
